...
State as of August 2026: **809 tests at 100% line + branch coverage**, ruff clean, and the mutation kill-property enforced on every module. CI is green on Python 3.11–3.14; the nightly mutation gate is green on the Linux runner. The desktop checkout and GitHub are the same repository, tracking `origin/main`.
...
The full suite (809 tests) runs in about seven seconds, so run it after
every change:
...
| `SEARCH_PROVIDERS` | `duckduckgo` | Comma-separated search providers (`duckduckgo`, `courtlistener`, `bva`, `bvasitemap`) |
...
Typos in `SEARCH_PROVIDERS` are warned about at startup and dropped; `--show-config` prints both `search_providers` (raw) and `effective_search_providers` (what will actually run). Valid names: `duckduckgo`, `courtlistener`, `bva`, `bvasitemap`.
